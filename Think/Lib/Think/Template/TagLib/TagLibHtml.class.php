...
class TagLibHtml extends ThinkTemplateCompiler{
    // 标签定义
    ...
    public function _grid($attr)
    {
        ...
        if(isset($tag['actionlist'])) {
            if(substr($tag['actionlist'],0,1)=='$') {dump($tag['actionlist']);
                $actionlist   = $this->tpl->get(substr($tag['actionlist'],1));dump($actionlist);
            }else {
    ...
    public function _list($attr)
    {
        ...
        $key         =  !empty($tag['key'])?true:false;
        $sort      = (!empty($tag['sort']) && $tag['sort']=='false')?false:true;
