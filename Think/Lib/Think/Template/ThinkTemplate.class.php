...
class ThinkTemplate {
    ...
    public function fetch($templateFile,$templateVar='')
    {
        ...
        if (!check_tmpl_cache($templateFile,$tmplCacheFile)) // 判断缓存是否有效
    ...
    protected function loadTemplate($templateFile,$tmplCacheFile) {
        ...
        // 检测缓存目录
        if(!is_dir($this->config['cache_path']))   mk_dir($this->config['cache_path']);
        //重写Cache文件
        if( false === file_put_contents($tmplCacheFile,trim($tmplContent)))
            throw_exception('模板缓存写入失败！');
    }
    // 模板编译
