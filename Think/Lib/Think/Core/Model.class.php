...
class Model extends Think
{
     ...
     * @access public
     +----------------------------------------------------------
     */
    public function __construct($name='',$db_tag='default',$config='')
    {
        // 模型初始化
        $this->_initialize();
        // 获取模型名称
        ...
        // 设置表前缀
          ...
          $this->tablePrefix = $this->tablePrefix?$this->tablePrefix:$this->db->config['prefix'];
          $this->tableSuffix = $this->tableSuffix?$this->tableSuffix:$this->db->config['suffix'];
        // 字段检测
    ...
    public function select($options=array()) {
        ...
        if(empty($resultSet)) { // 查询结果为空
            ...
            array_walk($resultSet,array($this,'checkSerializeField'));
        }
     ...
     */
     public function find($options=array()) {
        ...
        if(empty($resultSet)) {// 查询结果为空
            ...
            $resultSet[0]   =  $this->checkSerializeField($resultSet[0]);
        }
     ...
     * @return mixed
     +----------------------------------------------------------
     */
    public function query($sql,$statment=0)
    {
        if(!empty($sql)) {
            if(strpos($sql,'__TABLE__'))
                $sql    =   str_replace('__TABLE__',$this->getTableName(),$sql);
 
            return $this->db->query($sql,$statment);
        }else{
            return false;
        }
    }
   ...
   /**
    ...
    public function mysqlFetchArray($sql)
    {
   		...
   		if(!empty($sql)) {
            if(strpos($sql,'__TABLE__'))
                $sql    =   str_replace('__TABLE__',$this->getTableName(),$sql);
            return $this->db->mysqlFetchArray($sql);
        }else{
            return false;
        }
    }
     ...
     * @param array $sqls
     */
    public function trans_sql($sqls)
    {
    	if(empty($sqls)) return false;
    	return $this->db->trans_sql($sqls);
    }
   ...
   /**
     ...
     * @return mixed
     +----------------------------------------------------------
     */
    public function master_query($sql,$type='all')
    {
    	if(!empty($sql)) {
            if(strpos($sql,'__TABLE__'))
                $sql    =   str_replace('__TABLE__',$this->getTableName(),$sql);
            return $this->db->master_query($sql,$type);
        }else{
            return false;
        }
    }
    ...
    {
    	if(!empty($table)&&!empty($field_values)) { 
            return $this->db->autoExecute($table, $field_values, $mode = 'INSERT', $where = '', $querymode = '');
        }else{
            return false;
        }
    }
     ...
     * @return string
     +----------------------------------------------------------
     */
    public function db($linkNum='default',$config=''){
     
        static $_db = array();  
	         $linkNum=$linkNum?$linkNum:'default';
	        if($config==''|| empty($config)){  
	        	$db_configs=C('DB_CONFIGS'); 
	            $config=$db_configs[$linkNum];
	        }
     ...
     * @access public
     +----------------------------------------------------------
     * @param resource $queryStatment 名称 
     +----------------------------------------------------------
     * @return Model
     +----------------------------------------------------------
     */
    
    public function fetch_array($queryStatment)
    {
    	if(!$queryStatment) return false;
    	if($this->db->config['dbms']!='mysql'){
     		return $this->db->fetch_array( $queryStatment);
    	}else{
    		return mysql_fetch_array( $queryStatment,MYSQL_ASSOC);
    	}
