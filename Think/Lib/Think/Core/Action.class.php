...
{//类定义开始
   ...
   /**
    ...
    public function __construct() {
        ...
        //模版主题名称
        define('TEMPLATE_NAME',get_template_theme());
        // 当前模版路径
        define('TEMPLATE_PATH',C('TMPL_PATH').TEMPLATE_NAME.(defined('GROUP_NAME')?'/'.GROUP_NAME:''));
        // 自动识别的当前操作模板文件
        C('TMPL_FILE_NAME',TEMPLATE_PATH.'/'.MODULE_NAME.'/'.ACTION_NAME.C('TMPL_TEMPLATE_SUFFIX'));
        // URL访问的项目模板目录
        define('APP_TMPL_PATH', __ROOT__.'/'.APPS_PATH.APP_NAME.'/Tpl/'.TEMPLATE_NAME);
        //控制器初始化
   ...
   /**
    ...
    protected function show($content,$charset='',$contentType='') {
        $this->view->show($content,$charset,$contentType);
    }
    ...
    public function __call($method,$args) {
        if( 0 === strcasecmp($method,ACTION_NAME)) {
            ...
            if(method_exists($this,'_empty')) {
                ...
                if(file_iexists(C('TMPL_FILE_NAME')))
                    $this->display();
                else
                    // 抛出异常
